...
function App() {
  ...
  const [positions, setPositions] =
    useState<Record<AppId, WindowPos>>(initialPositions);
  const [drag, setDrag] = useState<DragState | null>(null);
  const [resize, setResize] = useState<ResizeState | null>(null);
  const [sizes, setSizes] = useState<Record<AppId, WindowSize>>(initialSizes);
  const [restoreBounds, setRestoreBounds] =
    useState<Record<AppId, WindowBounds | null>>(initialRestoreBounds);
  ...
  const [activeGalleryPhoto, setActiveGalleryPhoto] =
  ...
  const [windows, setWindows] =
    useState<Record<AppId, AppWindow>>(initialWindows);

  const [widgetSettings, setWidgetSettings] = useState<
    import("./app/types").WidgetSettings
  >(() => {
    const raw = window.localStorage.getItem("w11-widgets");
    if (raw) {
      try {
  ...
  // Whenever widgetSettings change, persist them
  useEffect(() => {
    window.localStorage.setItem("w11-widgets", JSON.stringify(widgetSettings));
  }, [widgetSettings]);
    ...
    function onResize() {
      ...
      (Object.keys(sizes) as AppId[]).forEach((id) => {
        ...
        const maxHeight = Math.max(
          minHeight,
          nextViewport.height - nextTaskbarReservedHeight - 12,
        );
        const width = Math.min(Math.max(minWidth, sizes[id].width), maxWidth);
        const height = Math.min(
          Math.max(minHeight, sizes[id].height),
          maxHeight,
        );
      ...
      (Object.keys(positions) as AppId[]).forEach((id) => {
        ...
        const maxY = Math.max(
          0,
          nextViewport.height - size.height - nextTaskbarReservedHeight,
        );
  ...
  function pushTerminalLines(
    lines: Array<{ text: string; tone: TerminalLine["tone"] }>,
  ) {
  ...
  function runTerminalCommand(rawInput: string) {
    ...
    pushTerminalLines([
      { text: `C:/Users/${userName}> ${commandText}`, tone: "input" },
    ]);
    ...
    const command =
      aliases[rawCommand.toLowerCase()] ?? rawCommand.toLowerCase();

    const pushOutput = (...text: string[]) =>
      pushTerminalLines(
        text.map((line) => ({ text: line, tone: "output" as const })),
      );
    const pushError = (text: string) =>
      pushTerminalLines([{ text, tone: "error" }]);
    const pushSystem = (...text: string[]) =>
      pushTerminalLines(
        text.map((line) => ({ text: line, tone: "system" as const })),
      );
    ...
    switch (command) {
      ...
      case "history":
        pushOutput(
          ...(terminalHistory.length
            ? terminalHistory.map((entry, index) => `${index + 1}. ${entry}`)
      ...
      case "man": {
        ...
        if (!topic) {
          ...
          pushOutput(
            "open <app-name>  # about/projects/gallery/blogs/jobs/timeline/contact/settings/terminal",
          );
        } else {
      ...
      case "apps":
        pushOutput(
          "Installed apps: about, projects, gallery, blogs, jobs, timeline, contact, settings, terminal",
        );
        break;
      case "open": {
      ...
      case "theme": {
        ...
        if (sub === "list") {
          pushOutput(
            `Themes: ${themeOptions.map((item) => item.id).join(", ")}`,
          );
        } else if (sub === "next") {
          toggleTheme();
      ...
      case "fortune": {
        const fortunes = [
          ...
          "Small commits build big products.",
          "Data tells stories if you ask the right questions.",
        ];
  ...
  function navigateTerminalHistory(direction: "up" | "down") {
    ...
    setTerminalHistoryIndex((prev) => {
      if (direction === "up") {
        const nextIndex =
          prev === -1 ? terminalHistory.length - 1 : Math.max(0, prev - 1);
  ...
  function autocompleteTerminalInput() {
    ...
    if (value.includes(" ")) {
      ...
      if (
        command.toLowerCase() === "theme" &&
        rest.length === 2 &&
        rest[0].toLowerCase() === "set"
      ) {
        const partial = rest[1].toLowerCase();
        const matches = themeOptions
          .map((item) => item.id)
    ...
    if (matches.length > 1) {
      pushTerminalLines([
        { text: `Suggestions: ${matches.join(", ")}`, tone: "system" },
      ]);
    }
  }
  ...
  useEffect(() => {
    ...
    void a.play().catch(() => {
      setMuted(true);
      setSoundToastText(
        "Sound is blocked by the browser. Click Enable sound to start music.",
      );
  ...
  async function enableSoundFromUserGesture() {
    ...
    try {
      ...
      setMuted(true);
      setSoundToastText(
        "Sound is blocked. Try interacting with the page and click Enable sound again.",
      );
      setSoundToastOpen(true);
    }
  }
  ...
  function toggleTheme() {
    setTheme((prev) => {
      const currentIndex = themeOptions.findIndex(
        (themeOption) => themeOption.id === prev,
      );
      const nextIndex =
  ...
  useEffect(() => {
    ...
    function onMove(e: PointerEvent) {
      ...
      const maxWidth = Math.max(
        minWidth,
        window.innerWidth - positions[r.id].x,
      );
      const maxHeight = Math.max(
        minHeight,
        window.innerHeight - taskbarReservedHeight - positions[r.id].y,
      );

      const width = Math.min(Math.max(minWidth, r.startWidth + dx), maxWidth);
      const height = Math.min(
        Math.max(minHeight, r.startHeight + dy),
        maxHeight,
      );
    ...
    () => [
      {
        key: "about",
        label: "About",
        description: "Profile, education and skills",
        type: "app" as const,
      ...
      },
      {
        key: "projects",
        label: "Projects",
        description: "AI, NLP and data engineering work",
        type: "app" as const,
      ...
      },
      {
        key: "gallery",
        label: "Gallery",
        description: "Photos loaded from /public/gallery",
        type: "app" as const,
        appId: "gallery" as const,
      },
      {
        key: "blogs",
        label: "Blogs",
        description: "Posts from kaus98.github.io",
        type: "app" as const,
        appId: "blogs" as const,
      },
      {
        key: "jobs",
        label: "Jobs",
        description: "Experience and internships",
        type: "app" as const,
        appId: "jobs" as const,
      },
      {
        key: "timeline",
        label: "Timeline",
        description: "Career milestones and dated journey",
        type: "app" as const,
        appId: "timeline" as const,
      },
      {
        key: "contact",
        label: "Contact",
      ...
      },
      {
        key: "settings",
        label: "Settings",
        description: "Theme, sound and time controls",
        type: "app" as const,
      ...
      },
      {
        key: "terminal",
        label: "Terminal",
        description: "Command line tools and shortcuts",
        type: "app" as const,
  ...
  const filteredPinnedApps = pinnedApps.filter(
    (app) =>
      app.id !== "terminal" &&
      app.label.toLowerCase().includes(normalizedStartQuery),
  );
  const filteredAllOptions = allOptions.filter(
    (item) =>
      item.label.toLowerCase().includes(normalizedStartQuery) ||
      item.description.toLowerCase().includes(normalizedStartQuery),
  );

  const about = portfolioData.about;
  const contact = portfolioData.contact;
  const availability = contact.availability;
  const timelineItems = [...portfolioData.timeline].sort(
  ...
  const searchedJobs = jobExperiences.filter((job) => {
    ...
    const haystack =
      `${job.company} ${job.role} ${job.period ?? ""} ${job.highlights.join(" ")}`.toLowerCase();
  ...
  const filteredProjectCards = projectCards.filter((project) => {
    if (!normalizedQuery) return true;
    const haystack =
      `${project.title} ${project.subtitle} ${project.description}`.toLowerCase();
    return haystack.includes(normalizedQuery);
  });
  ...
  const searchWindowStyle = {
    ...
    left: Math.max(
      8,
      Math.floor((viewport.width - Math.min(760, viewport.width - 16)) / 2),
    ),
    top: viewport.width < 700 ? 8 : 48,
    width: Math.min(760, viewport.width - 16),
    height: Math.max(
      240,
      Math.min(500, viewport.height - taskbarReservedHeight - 16),
    ),
  };
  ...
  return (
    ...
    >
      ...
      {widgetSettings.showTypewriter && <TypewriterBackground />}
      {isSleeping && (
        <div
      ...
      <audio
        ...
        onError={() => {
          ...
          setAudioError(true);
          setSoundToastText(
            "Music file not found. Add /public/audio/background.mp3 to enable sound.",
          );
          setSoundToastOpen(true);
        }}
      />
        ...
        >
          ...
          <div className="toast-actions">
            <button
              ...
              type="button"
              onClick={() => void enableSoundFromUserGesture()}
              disabled={audioError}
            >
              Enable sound
            </button>
            <button
              className="toast-btn"
              type="button"
              onClick={() => setSoundToastOpen(false)}
            >
              Dismiss
            </button>
          </div>
